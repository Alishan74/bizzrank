#!/usr/bin/env bash
# BizzRank AI — AI Citation Intelligence
# cd /workspaces/bizzrank/bizzrank-v10 && bash ai_citations.sh
set -e
ROOT="$(pwd)"
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo " BizzRank AI — Citation Intelligence"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
mkdir -p "$ROOT/apps/api/src/domains/aivisibility"

# ── 1. Write AICitationService.ts ────────────────────────────
echo "  [1/5] AICitationService.ts"
cat > "$ROOT/apps/api/src/domains/aivisibility/AICitationService.ts" << 'TSEOF'
/**
 * ============================================================
...
# ── 2. Patch AIVisibilityService to call citation analysis ────
echo "  [2/5] Integrating citations into AIVisibilityService"
python3 - "$ROOT" << 'PYEOF'
import sys, os
root = sys.argv[1]
path = root + '/apps/api/src/domains/aivisibility/AIVisibilityService.ts'
if not os.path.exists(path):
    print("  ⚠ AIVisibilityService.ts not found — run ai_visibility_v2.sh first")
    sys.exit(0)
with open(path) as f: src = f.read()

# Add citation import at top
if 'AICitationService' not in src:
    src = src.replace(
        "import { db } from '../../infrastructure/database/SupabaseClient.js';",
        "import { db } from '../../infrastructure/database/SupabaseClient.js';\nimport { aiCitationService } from './AICitationService.js';"
    )

# Add citation_intelligence to AIVisibilityReport interface
if 'citationIntelligence' not in src:
    src = src.replace(
        "  promptResults:   AggregatedPromptResult[];",
        "  promptResults:   AggregatedPromptResult[];\n  citationIntelligence?: any;  // populated after citations analysis"
    )

# Call citation analysis after saving main report
old_save = "    logger.info('[AIVisibility] Complete', {"
new_save = """    // ── Run citation intelligence analysis ──────────────────
...
if 'citationIntelligence' not in src or 'analyzeCitations' not in src:
    src = src.replace(old_save, new_save)
    print("  ✓ AIVisibilityService.ts citations integration added")
else:
    print("  ✓ Citations already integrated")

with open(path, 'w') as f: f.write(src)
PYEOF

# ── 3. Update aiVisibility route to serve citation data ──────
echo "  [3/5] aiVisibility.ts route — add citations endpoint"
python3 - "$ROOT" << 'PYEOF'
import sys, os
root = sys.argv[1]
path = root + '/apps/api/src/api/routes/aiVisibility.ts'
if not os.path.exists(path):
    print("  ⚠ aiVisibility.ts route not found")
    sys.exit(0)
with open(path) as f: src = f.read()

if 'citations' not in src:
    src = src.replace(
    ...
    # Add citations endpoint before export
    src = src.replace(
...
export default router;"""
    ...
    with open(path, 'w') as f: f.write(src)
    print("  ✓ aiVisibility.ts citations endpoints added")
else:
    print("  ✓ Already has citations endpoint")
PYEOF

# ── 4. Update api.ts frontend ─────────────────────────────────
echo "  [4/5] api.ts — add citation API methods"
python3 - "$ROOT" << 'PYEOF'
import sys
root = sys.argv[1]
...
if 'citations' not in src:
    # Find aiVisibilityApi block and extend it
    src = src.replace(
        "export const aiVisibilityApi = {",
        """export const aiVisibilityApi = {"""
    )
    # Add after existing aiVisibilityApi
    src = src.replace(
  ...
  citationSources:(sector: string)     => api.get('/ai-visibility/citation-sources?sector=' + sector),
    ...
    with open(path, 'w') as f: f.write(src)
    print("  ✓ api.ts citation methods added")
else:
    print("  ✓ api.ts already has citations")
PYEOF

# ── 5. Write Citations tab component ─────────────────────────
echo "  [5/5] AIVisibility.tsx — Citations tab"
cat > "$ROOT/apps/frontend/src/components/CitationsTab.tsx" << 'JSEOF'
/**
...
# ── Add Citations tab to AIVisibility.tsx ────────────────────
echo "  Adding Citations tab to AIVisibility.tsx..."
python3 - "$ROOT" << 'PYEOF'
import sys, os
root = sys.argv[1]
path = root + '/apps/frontend/src/pages/AIVisibility.tsx'
if not os.path.exists(path):
    print("  ⚠ AIVisibility.tsx not found — run ai_visibility_v2.sh first")
    sys.exit(0)
with open(path) as f: src = f.read()

# Add CitationsTab import
if 'CitationsTab' not in src:
    src = src.replace(
        "import { bizApi, aiVisibilityApi } from '../lib/api';",
        "import { bizApi, aiVisibilityApi } from '../lib/api';\nimport CitationsTab from '../components/CitationsTab';"
    )
    # Add citations to TABS array
    src = src.replace(
        "    { id:'prompts',   label:'Prompts'   },",
        "    { id:'prompts',   label:'Prompts'   },\n    { id:'citations', label:'Citations ✨' },"
    )
    # Add citations tab content after the prompts tab
    old_footer = "    </div>\n    </>\n  );\n}"
    new_footer = """    </div>
    ...
    with open(path, 'w') as f: f.write(src)
    print("  ✓ AIVisibility.tsx Citations tab added")
else:
    print("  ✓ Citations tab already present")
PYEOF

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo " AI Citation Intelligence complete"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo " Files created/updated:"
echo "   ✓ [NEW] AICitationService.ts  — 1,087 lines"
echo "   ✓ [UPD] AIVisibilityService.ts — citation analysis called after check"
echo "   ✓ [UPD] aiVisibility.ts route — /citations + /citation-sources endpoints"
echo "   ✓ [UPD] api.ts                — citations() + citationSources() methods"
echo "   ✓ [NEW] CitationsTab.tsx      — full citation intelligence UI"
echo "   ✓ [UPD] AIVisibility.tsx      — Citations tab added"
echo ""
echo " Run migration/010-ai-citations.sql in Supabase"
